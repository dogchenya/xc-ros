...
#ifndef _ROBOTRPCCLIENT_H_
#define _ROBOTRPCCLIENT_H_
...
namespace RobotRpc {
  ...
  //! A class to send Robot RPC requests to a server and return the results.
  class ROBOTRPCPP_DECL RobotRpcClient : public RobotRpcSocketSource {
  public:
    // Static data
    ...
    RobotRpcClient(const char* host, int port, const char* uri=0);

    //! Destructor
    virtual ~RobotRpcClient();
    ...
    // RobotRpcSource interface implementation
    //! Close the connection
    virtual void close();

    //! Handle server responses. Called by the event dispatcher during execute.
    //!  @param eventType The type of event that occurred. 
    //!  @see RobotRpcDispatch::EventType
  ...
  public:
    ...
    // The Robot-encoded request, http header of response, and response Robot
    std::string _request;
    std::string _header;
