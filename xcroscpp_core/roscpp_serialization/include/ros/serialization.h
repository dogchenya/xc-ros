...
#ifndef _XCROS_CORE_SERIALIZATION_H_
#define _XCROS_CORE_SERIALIZATION_H_
...
class ROSCPP_SERIALIZATION_DECL StreamOverrunException : public xcros::Exception
{
public:
...
/**
 * \brief Serializer specialized for xcros::Time
 */
template<>
struct Serializer<xcros::Time>
{
  template<typename Stream>
  inline static void write(Stream& stream, const xcros::Time& v)
  {
    stream.next(v.sec);
    stream.next(v.nsec);
  }
  ...
  template<typename Stream>
  inline static void read(Stream& stream, xcros::Time& v)
  {
    stream.next(v.sec);
    stream.next(v.nsec);
  }

  inline static uint32_t serializedLength(const xcros::Time&)
  {
    return 8;
  }
};
...
/**
 * \brief Serializer specialized for xcros::Duration
 */
template<>
struct Serializer<xcros::Duration>
{
  template<typename Stream>
  inline static void write(Stream& stream, const xcros::Duration& v)
  {
    stream.next(v.sec);
    stream.next(v.nsec);
  }
  ...
  template<typename Stream>
  inline static void read(Stream& stream, xcros::Duration& v)
  {
    stream.next(v.sec);
    stream.next(v.nsec);
  }

  inline static uint32_t serializedLength(const xcros::Duration&)
  {
    return 8;
  }
};
