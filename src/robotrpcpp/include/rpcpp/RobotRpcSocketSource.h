...
#ifndef _ROBOTRPCSOCKETSOURCE_H_
#define _ROBOTRPCSOCKETSOURCE_H_
...
namespace RobotRpc {
  ...
  class ROBOTRPCPP_DECL RobotRpcSocketSource {
  public:
    //! Constructor
    //!  @param fd The socket file descriptor to monitor.
    //!  @param deleteOnClose If true, the object deletes itself when close is called.
    RobotRpcSocketSource(int fd = -1, bool deleteOnClose = false);

    //! Destructor
    virtual ~RobotRpcSocketSource();
  ...
  private:
    ...
    // In the server, a new source (RobotRpcServerConnection) is created
...
#endif //_ROBOTRPCSOCKETSOURCE_H_
