...
namespace RobotRpc {
  ...
  void XmlRpcValue::assertTypeOrInvalid(Type t)
  {
    if (_type == TypeInvalid)
    {
      ...
      throw RobotRpcException("type error");
  }

  void XmlRpcValue::assertArray(int size) const
  {
    if (_type != TypeArray)
      throw RobotRpcException("type error: expected an array");
    else if (int(_value.asArray->size()) < size)
      throw RobotRpcException("range error: array index too large");
  }
  ...
  {
    if (_type != TypeStruct)
      throw RobotRpcException("type error: expected a struct");
  }
  ...
  bool XmlRpcValue::fromXml(std::string const& valueXml, int* offset)
  {
    ...
    invalidate();
    if ( ! RobotRpcUtil::nextTagIs(VALUE_TAG, valueXml, offset))
      return false;       // Not a value, offset not updated

	int afterValueOffset = *offset;
    std::string typeTag = RobotRpcUtil::getNextTag(valueXml, offset);
    bool result = false;
    ...
    if (result)  // Skip over the </value> tag
      RobotRpcUtil::findTag(VALUE_ETAG, valueXml, offset);
  ...
  bool XmlRpcValue::stringFromXml(std::string const& valueXml, int* offset)
  {
    ...
    _type = TypeString;
    _value.asString = new std::string(RobotRpcUtil::xmlDecode(valueXml.substr(*offset, valueEnd-*offset)));
  ...
  std::string XmlRpcValue::stringToXml() const
  {
    ...
    xml += RobotRpcUtil::xmlEncode(*_value.asString);
    //xml += STRING_ETAG;
    xml += VALUE_ETAG;
    return xml;
  }
  ...
  {
    if ( ! RobotRpcUtil::nextTagIs(DATA_TAG, valueXml, offset))
      return false;
    ...
    (void) RobotRpcUtil::nextTagIs(DATA_ETAG, valueXml, offset);
    return true;
  }
  ...
  bool XmlRpcValue::structFromXml(std::string const& valueXml, int* offset)
  {
    ...
    while (RobotRpcUtil::nextTagIs(MEMBER_TAG, valueXml, offset)) {
      // name
      const std::string name = RobotRpcUtil::parseTag(NAME_TAG, valueXml, offset);
      // value
      ...
      (void) RobotRpcUtil::nextTagIs(MEMBER_ETAG, valueXml, offset);
    }
    return true;
  }
  ...
  std::string XmlRpcValue::structToXml() const
  {
    ...
    for (it=_value.asStruct->begin(); it!=_value.asStruct->end(); ++it) {
      xml += MEMBER_TAG;
      xml += NAME_TAG;
      xml += RobotRpcUtil::xmlEncode(it->first);
      xml += NAME_ETAG;
      xml += it->second.toXml();
      xml += MEMBER_ETAG;
    }
