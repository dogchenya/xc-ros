...
#ifndef _XCROS_CORE_MESSAGE_TRAITS_H_
#define _XCROS_CORE_MESSAGE_TRAITS_H_
...
struct TimeStamp
{
  static xcros::Time* pointer(M& m) { (void)m; return 0; }
  static xcros::Time const* pointer(const M& m) { (void)m; return 0; }
};

template<typename M>
struct TimeStamp<M, typename boost::enable_if<HasHeader<M> >::type >
{
  static xcros::Time* pointer(typename boost::remove_const<M>::type &m) { return &m.header.stamp; }
  static xcros::Time const* pointer(const M& m) { return &m.header.stamp; }
  static xcros::Time value(const M& m) { return m.header.stamp; }
};
 ...
 */
template<typename M>
inline xcros::Time* timeStamp(M& m)
{
 ...
 */
template<typename M>
inline xcros::Time const* timeStamp(const M& m)
{
  return TimeStamp<typename boost::remove_reference<typename boost::remove_const<M>::type>::type>::pointer(m);
}
