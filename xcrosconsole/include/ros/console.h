#ifndef _XCROSCONSOLE_CONSOLE_H_
#define _XCROSCONSOLE_CONSOLE_H_
...
public:
  ...
  virtual void log(::xcros::console::Level level, const char* str, const char* file, const char* function, int line) = 0;
...
struct Token
{
   ...
   * @param  line
   */
  virtual std::string getString(void*, ::xcros::console::Level, const char*, const char*, const char*, int) = 0;
};
 ...
 *
 * If you're going to be using log4cxx or any of the ::xcros::console functions, and need the system to be initialized, use the
...
#define ROSCONSOLE_AUTOINIT \
  do \
  { \
    if (ROS_UNLIKELY(!::xcros::console::g_initialized)) \
    { \
      ::xcros::console::initialize(); \
    } \
  } while(false)

#define ROSCONSOLE_DEFINE_LOCATION(cond, level, name) \
  ROSCONSOLE_AUTOINIT; \
  static ::xcros::console::LogLocation __rosconsole_define_location__loc = {false, false, ::xcros::console::levels::Count, 0}; /* Initialized at compile-time */ \
...
#define ROSCONSOLE_PRINT_AT_LOCATION_WITH_FILTER(filter, ...) \
    ::xcros::console::print(filter, __rosconsole_define_location__loc.logger_, __rosconsole_define_location__loc.level_, __FILE__, __LINE__, __ROSCONSOLE_FUNCTION__, __VA_ARGS__)
...
#define ROSCONSOLE_PRINT_STREAM_AT_LOCATION_WITH_FILTER(filter, args) \
  ...
  { \
    ...
    ::xcros::console::print(filter, __rosconsole_define_location__loc.logger_, __rosconsole_define_location__loc.level_, __rosconsole_print_stream_at_location_with_filter__ss__, __FILE__, __LINE__, __ROSCONSOLE_FUNCTION__); \
  } while (0)
 ...
 * \param level One of the levels specified in ::xcros::console::levels::Level
...
#define ROS_LOG_STREAM_THROTTLE(period, level, name, args) \
  ...
  { \
    ...
    ::xcros::Time __ros_log_stream_throttle__now__ = ::xcros::Time::now(); \
...
#define ROS_LOG_DELAYED_THROTTLE(period, level, name, ...) \
  ...
  { \
    ROSCONSOLE_DEFINE_LOCATION(true, level, name); \
    ::xcros::Time __ros_log_delayed_throttle__now__ = ::xcros::Time::now(); \
...
#define ROS_LOG_STREAM_DELAYED_THROTTLE(period, level, name, args) \
  ...
  { \
    ROSCONSOLE_DEFINE_LOCATION(true, level, name); \
    ::xcros::Time __ros_log_stream_delayed_throttle__now__ = ::xcros::Time::now(); \
